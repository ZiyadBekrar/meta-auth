...
- `GET /meta-auth/callback`: exchanges the returned `code` for a long-lived user token, fetches full `/me/accounts` payload (all managed pages with their page access tokens), and stores that JSON in Google Secret Manager (no token is displayed)
